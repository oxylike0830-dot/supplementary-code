...
class Curve(arrays.Vectorspace):
    ...
    def oriented_transport_frames(self, origin, jVector, landmark_hat, return_index=True):
        ...
        ijk_real = arrays.Basis(i_real, j_real, z_new)
        
        newTransportFrames = np.zeros_like(transportFrames)
        for i, frame in enumerate(transportFrames):
            newTransportFrames[i] = arrays.Basis(frame @ ijk_new @ ijk_real)
        if return_index:
